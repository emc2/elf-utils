...
//! ];
//!
//! assert_eq!(required_bytes(ELF_NOTES_CONTENTS.iter()), 72)
//! ```
//!
...
#[cfg(feature = "builder")]
/// A builder object for [Notes].
#[derive(Clone)]
pub struct NotesBuilder<'a, B: ByteOrder> {
    byteorder: PhantomData<B>,
...
#[inline]
fn get_size<'a, B>(data: &'a [u8]) -> Option<usize>
    where B: ByteOrder {
...
fn create_split_raw<'a, 'b, B, I>(buf: &'b mut [u8], notes: I) ->
    Result<(&'b mut [u8], &'b mut [u8]), ()>
    where I: Iterator,
          I::Item: Borrow<NoteData<'a>>,
          B: ByteOrder {
    let mut idx = 0;

    for note in notes {
        let note = note.borrow();
        let namesize = note.name.len();
        let descsize = note.desc.len();
        ...
        if buf.len() >= size + idx {
            ...
            (&mut buf[idx + name_start .. idx + name_end])
                .clone_from_slice(&note.name[0..]);
            (&mut buf[idx + desc_start .. idx + desc_end])
                .clone_from_slice(&note.desc[0..]);
...
pub fn required_bytes<'a, I>(notes: I) -> usize
    where I: Iterator<Item = &'a NoteData<'a>> {
    let mut size = 0;
...
impl<'a, B> Notes<'a, B>
    ...
    /// let mut buf = [0; 80];
    /// let res: Result<(Notes<'_, LittleEndian>, &'_ mut [u8]), ()> =
    ///     Notes::create_split(&mut buf[0..], NOTES_CONTENTS.iter());
    /// let (notes, buf) = res.unwrap();
    ///
    /// assert_eq!(buf.len(), 8);
    ///
    ...
    /// ```
    #[inline]
    pub fn create_split<'b, I>(buf: &'b mut [u8], notes: I) ->
        Result<(Notes<'b, B>, &'b mut [u8]), ()>
        where I: Iterator,
              I::Item: Borrow<NoteData<'a>>,
              B: ByteOrder {
        let byteorder: PhantomData<B> = PhantomData;

        match create_split_raw::<B, I>(buf, notes) {
    ...
    /// let mut buf = [0; 80];
    /// let notes: Notes<'_, LittleEndian> =
    ///     Notes::create(&mut buf[0..], NOTES_CONTENTS.iter())
    ///     .unwrap();
    /// let mut iter = notes.iter();
    ///
    /// assert_eq!(iter.next(), Some(NoteData { kind: 1, name: &NOTE_1_NAME,
    ...
    /// ```
    #[inline]
    pub fn create<'b, I>(buf: &'b mut [u8], notes: I) ->
        Result<Notes<'b, B>, ()>
        where I: Iterator,
              I::Item: Borrow<NoteData<'a>>,
              B: ByteOrder {
...
#[cfg(feature = "builder")]
impl<'a, B> NotesBuilder<'a, B>
    where B: ByteOrder {
    /// Create a new `NotesBuilder`.
    pub fn new() -> NotesBuilder<'a, B> {
        NotesBuilder { byteorder: PhantomData, notes: Vec::new() }
    }

    /// Create a new `NotesBuilder` with a size hint.
    pub fn with_capacity(size: usize) -> NotesBuilder<'a, B> {
        NotesBuilder { byteorder: PhantomData, notes: Vec::with_capacity(size) }
    }

    /// Add a [NoteData] to this `NotesBuilder`.
    pub fn add(&mut self, note: NoteData<'a>) {
    ...
    pub fn size(&self) -> usize {
        ...
        for note in &self.notes {
            out += 4 + note.name.len() + note.desc.len();
        }
    ...
    /// Attempt to create a `Notes` in `buf` containing the note objects
    /// in this `NotesBuilder`.
    ///
    /// This will write the note data into the buffer in the ELF
    /// format.  Returns the `Notes` if successful.
    pub fn build<'b>(&self, buf: &'b mut [u8]) -> Result<Notes<'b, B>, ()> {
        Notes::create(buf, self.notes.iter())
    }
    ...
    pub fn build_split<'b>(&self, buf: &'b mut [u8]) ->
        Result<(Notes<'b, B>, &'b mut [u8]), ()> {
        Notes::create_split(buf, self.notes.iter())
    }
}
...
impl<'a, B> Iterator for NotesIter<'a, B>
    ...
    fn next(&mut self) -> Option<NoteData<'a>> {
        ...
        match get_size::<B>(&self.data[start..]) {
            Some(size) => {
