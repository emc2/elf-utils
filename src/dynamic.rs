...
#[cfg(feature = "builder")]
pub struct DynamicBuilder<B: ByteOrder, Offsets: DynamicOffsets> {
    byteorder: PhantomData<B>,
    /// Buffered dynamic entries.
    dynamics: Vec<DynamicEntDataRaw<Offsets>>
}
...
#[cfg(feature = "builder")]
impl<B, Offsets> DynamicBuilder<B, Offsets>
    where B: ByteOrder,
          Offsets: DynamicOffsets {
    /// Create a new `DynamicBuilder`.
    pub fn new() -> DynamicBuilder<B, Offsets> {
        DynamicBuilder { byteorder: PhantomData, dynamics: Vec::new() }
    }

    /// Create a new `DynamicBuilder` with a size hint.
    pub fn with_capacity(size: usize) -> DynamicBuilder<B, Offsets> {
        DynamicBuilder { byteorder: PhantomData,
                         dynamics: Vec::with_capacity(size) }
    }

    /// Add a [RelData] to this `DynamicBuilder`.
    pub fn add(&mut self, dynamic: DynamicEntDataRaw<Offsets>) {
    ...
    pub fn size(&self) -> usize {
        self.dynamics.len() * Offsets::DYNAMIC_SIZE
    }

    /// Attempt to create a [Dynamic] in `buf` containing the dynamic
    /// entries in this `DynamicBuilder`.
    ///
    /// This will write the dynamic entry data into the buffer in the
    /// ELF format.  Returns the `Dynamic` if successful.
    pub fn build<'b>(&self, buf: &'b mut [u8]) ->
        Result<Dynamic<'b, B, Offsets>, ()> {
        Dynamic::create(buf, self.dynamics.iter())
    }
    ...
    ///
    /// This will write the dynamic entry data into the buffer in the
    /// ELF format.  Returns both the `Dynamic` and the remaining
    /// space if successful.
    pub fn build_split<'b>(&self, buf: &'b mut [u8]) ->
        Result<(Dynamic<'b, B, Offsets>, &'b mut [u8]), ()> {
        Dynamic::create_split(buf, self.dynamics.iter())
    }
}
