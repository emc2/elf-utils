...
#[cfg(feature = "builder")]
use crate::strtab::StrtabBuilderIdx;
#[cfg(feature = "builder")]
use crate::strtab::StrtabBuilderIdxMap;
...
#[cfg(feature = "builder")]
/// A builder object for [Symtab]s.
pub struct SymtabBuilder<Offsets: SymOffsets> {
    /// Buffered symbols.
    syms: Vec<SymData<StrtabBuilderIdx, Offsets::Half, Offsets>>
}
...
#[cfg(feature = "builder")]
impl<Offsets> SymtabBuilder<Offsets>
    where Offsets: SymOffsets {
    /// Create a new `SymtabBuilder`.
    pub fn new() -> SymtabBuilder<Offsets> {
        SymtabBuilder { syms: Vec::new() }
    }

    /// Create a new `SymtabBuilder` with a size hint.
    pub fn with_capacity(size: usize) -> SymtabBuilder<Offsets> {
        SymtabBuilder { syms: Vec::with_capacity(size) }
    }

    /// Add a [SymData] to this `SymtabBuilder`.
    pub fn add<'a>(&mut self, sym: &SymData<&'a str, Offsets::Half, Offsets>,
                   strtab: &mut StrtabBuilder<'a>) ->
        Result<Offsets::Word, ()> {
        match self.syms.len().try_into() {
            Ok(out) => {
                match sym.name {
                    Some(name) => {
                        let idx = strtab.add(name);

                        self.syms.push(SymData {
                            name: Some(idx), value: sym.value,
                            size: sym.size, kind: sym.kind,
                            bind: sym.bind, section: sym.section
                        })
                    },
                    None => {
                        self.syms.push(SymData {
                            name: None, value: sym.value,
                            size: sym.size, kind: sym.kind,
                            bind: sym.bind, section: sym.section
                        })
                    }
    ...
    pub fn size(&self) -> usize {
        self.syms.len() * Offsets::ST_ENT_SIZE
    }

    /// Attempt to create a `Symtab` in `buf` containing the symbols
    /// in this `SymtabBuilder`.
    ///
    /// This will write the symbol table data into the buffer in the
    /// proper format for the ELF class and byte order.  Returns both
    /// the `Symtab` and the remaining space if successful.
    #[inline]
    pub fn build_split<'a, B>(&self, buf: &'a mut [u8],
                              strtab: StrtabBuilderIdxMap<Offsets>) ->
        Result<(Symtab<'a, B, Offsets>, &'a mut [u8]), ()>
        where B: ByteOrder {
        Symtab::create_split(buf, self.syms.iter().map(|sym| {
            match &sym.name {
                Some(name) => {
                    SymData { name: Some(strtab[*name]),
                              value: sym.value, size: sym.size, kind: sym.kind,
                              bind: sym.bind, section: sym.section }
                },
                None => {
                    SymData { name: None, value: sym.value, size: sym.size,
                              kind: sym.kind, bind: sym.bind,
                              section: sym.section }
                }
            }
        }))
    ...
    /// proper format for the ELF class and byte order.
    #[inline]
    pub fn build<'a, B>(&self, buf: &'a mut [u8],
                        strtab: StrtabBuilderIdxMap<Offsets>) ->
        Result<Symtab<'a, B, Offsets>, ()>
        where B: ByteOrder {
        Symtab::create(buf, self.syms.iter().map(|sym| {
            match &sym.name {
                Some(name) => {
                    SymData { name: Some(strtab[*name]),
                              value: sym.value, size: sym.size, kind: sym.kind,
